...
use pyth_sdk_solana::PriceIdentifier;
...
pub use pyth_sdk_solana::{Price, PriceFeed};
...
#[repr(C)]
#[derive(Copy, Clone, Debug, PartialEq, Eq, type_layout::TypeLayout)]
pub struct GenericPriceAccount {
    /// pyth magic number
    pub magic: u32,
    /// program version
    pub ver: u32,
...
impl Default for GenericPriceAccount {
    fn default() -> Self {
        Self {
...
impl std::ops::Deref for GenericPriceAccount {
    type Target = ();
...
pub type PriceAccount = GenericPriceAccount;

/// Solana-specific Pyth account where the old 32-element publishers are present.
pub type SolanaPriceAccount = GenericPriceAccount;
...
#[cfg(target_endian = "little")]
unsafe impl Zeroable for GenericPriceAccount {}

#[cfg(target_endian = "little")]
unsafe impl Pod for GenericPriceAccount {}

impl GenericPriceAccount {
...
pub fn load_price_account(data: &[u8]) -> Result<&GenericPriceAccount, PythError> {
    let pyth_price =
        load::<GenericPriceAccount>(data).map_err(|_| PythError::InvalidAccountData)?;
